...
4. Open the section group link on the board (for example `ccsad-class`). The User groups list page is blocked for your user, so use the link. Open the Permissions tab. Open the policy `umak-lab-t0-observe` and choose the JSON tab. Notice that no statement allows `ec2:RunInstances`.
...
| Terminate an instance tagged `team=ccsad-g02` while signed in as `ccsad-g03` | | |
| Create an IAM user | | |
